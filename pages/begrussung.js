...
document.addEventListener("DOMContentLoaded", () => {
  const AUDIO_PATH = "../assets/audio/begrussung/";
  const audio = document.getElementById("lessonAudio");
  const soundToggle = document.getElementById("soundToggle");
  const toast = document.getElementById("toast");
  const completedSteps = new Set(JSON.parse(localStorage.getItem("begrussungSteps") || "[]"));
  ...
  const markComplete = (step) => {
    ...
    localStorage.setItem("begrussungSteps", JSON.stringify([...completedSteps]));
    updateProgress();
  };
