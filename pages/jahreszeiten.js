...
const seasons = {
  fruehling:{name:"der FrÃ¼hling",ro:"primÄƒvara",image:"fruehling.webp",icon:"fruehling.svg",months:["MÃ¤rz","April","Mai"],sentence:"Im FrÃ¼hling blÃ¼hen die Blumen.",translation:"PrimÄƒvara Ã®nfloresc florile.",audio:"fruehling.mp3",sentenceAudio:"fruehling-satz.mp3"},
  ...
  herbst:{name:"der Herbst",ro:"toamna",image:"herbst.webp",icon:"herbst.svg",months:["September","Oktober","November"],sentence:"Im Herbst fallen die BlÃ¤tter.",translation:"Toamna cad frunzele.",audio:"herbst.mp3",sentenceAudio:"herbst-satz.mp3"},
  winter:{name:"der Winter",ro:"iarna",image:"winter.webp",icon:"winter.svg",months:["Dezember","Januar","Februar"],sentence:"Im Winter ist es kalt und es schneit.",translation:"Iarna este frig È™i ninge.",audio:"winter.mp3",sentenceAudio:"winter-satz.mp3"}
};

const ordinals = [
  ["Januar","erste"],["Februar","zweite"],["MÃ¤rz","dritte"],["April","vierte"],
  ["Mai","fÃ¼nfte"],["Juni","sechste"],["Juli","siebte"],["August","achte"],
  ["September","neunte"],["Oktober","zehnte"],["November","elfte"],["Dezember","zwÃ¶lfte"]
];
...
const objects = [
  ...
  ["objekt-kuerbis.webp","der KÃ¼rbis","herbst"],["objekt-regenkleidung.webp","die Regenjacke","herbst"],
  ["objekt-muetze.webp","die MÃ¼tze","winter"],["objekt-handschuhe.webp","die Handschuhe","winter"],
  ["objekt-schlitten.webp","der Schlitten","winter"],["objekt-schneemann.webp","der Schneemann","winter"]
];

const quizQuestions = [
  {q:"Wie viele Jahreszeiten hat ein Jahr?",a:["zwei","vier","zwÃ¶lf"],correct:1},
  {q:"Welche Monate gehÃ¶ren zum FrÃ¼hling?",a:["MÃ¤rz, April, Mai","Juni, Juli, August","Dezember, Januar, Februar"],correct:0},
  {q:"Welche Jahreszeit kommt nach dem Sommer?",a:["der Winter","der Herbst","der FrÃ¼hling"],correct:1},
  {q:"In welcher Jahreszeit schneit es?",a:["im Sommer","im FrÃ¼hling","im Winter"],correct:2},
  {q:"Der wievielte Monat ist MÃ¤rz?",a:["der zweite","der dritte","der vierte"],correct:1},
  {q:"August ist â€¦",a:["der achte Monat","der neunte Monat","der elfte Monat"],correct:0},
  {q:"Welche Monate gehÃ¶ren zum Herbst?",a:["April, Mai, Juni","September, Oktober, November","Januar, Februar, MÃ¤rz"],correct:1},
  {q:"Was passt zum Sommer?",a:["der Schlitten","die Sonnenbrille","die MÃ¼tze"],correct:1},
  {q:"Dezember ist â€¦",a:["der zehnte Monat","der elfte Monat","der zwÃ¶lfte Monat"],correct:2},
  {q:"Im FrÃ¼hling â€¦",a:["blÃ¼hen die Blumen","fallen die BlÃ¤tter","bauen wir einen Schneemann"],correct:0}
];
...
function playAudio(files,button,statusElement){
  ...
  let index=0;
  const tryNext=()=>{
    if(index>=candidates.length){if(button)button.classList.remove("playing");if(statusElement)statusElement.textContent="FiÈ™ierul audio nu a fost gÄƒsit. VerificÄƒ denumirea MP3.";return;}
...
function setImageWithFallback(image, candidates, altText) {
  ...
  function loadNext() {
    if (!sources.length) {
      ...
      image.title = "Imaginea sezonierÄƒ lipseÈ™te. VerificÄƒ denumirea fiÈ™ierului Ã®n GitHub.";
      return;
    }
...
function toggleWheel(){
  ...
  result.innerHTML=`<img src="${IMAGE}${s.image}" alt="${s.name}"><div class="wheel-result-content"><h3>${s.name}</h3><p class="translation">${s.ro}</p><p><strong>${s.months.join(" Â· ")}</strong></p><p>${s.sentence}</p><button class="audio-btn" type="button" data-audio="${s.sentenceAudio}"><img src="${ICON}audio.svg" alt=""> AnhÃ¶ren</button></div>`;
...
function setupObject(){
  ...
  document.getElementById("objectCard").innerHTML=`<img src="${IMAGE}${file}" alt="${escapeHtml(name)}" data-object-audio="${file.replace("objekt-","").replace(".webp",".mp3")}"><strong>${escapeHtml(name)}</strong><small>Bild berÃ¼hren und hÃ¶ren</small>`;
...
function renderPackedObjects(){
  document.getElementById("packedObjects").innerHTML=packedObjects.map(item=>`<button class="packed-item" type="button" data-object-audio="${item.audio}" aria-label="${escapeHtml(item.name)} hÃ¶ren"><img src="${IMAGE}${item.file}" alt="${escapeHtml(item.name)}"></button>`).join("");
}
function checkObject(button){
  ...
  setTimeout(()=>{objectIndex++;if(objectIndex<objects.length){setupObject();setFeedback(document.getElementById("objectFeedback"),"");}else{document.getElementById("objectCard").innerHTML="<strong>Der Koffer ist fertig!</strong><small>BerÃ¼hre die GegenstÃ¤nde im Koffer und hÃ¶re sie noch einmal.</small>";document.getElementById("objectAnswers").innerHTML="";setFeedback(document.getElementById("objectFeedback"),"Gut gemacht! Alle 16 GegenstÃ¤nde sind richtig eingeordnet.","success");unlockNext(6);}},600);
}
...
function checkDress(button){
  ...
  setTimeout(()=>{dressIndex++;if(dressIndex<4){setupDress();setFeedback(document.getElementById("dressFeedback"),"");}else{setFeedback(document.getElementById("dressFeedback"),"Super! Felix ist fÃ¼r jedes Wetter bereit.","success");unlockNext(7);}},650);
}

const sentenceWords=["Im","Herbst","fallen","die","BlÃ¤tter."];
let sentenceChoice=[];
...
function chooseWord(button){button.disabled=true;sentenceChoice.push(button.dataset.word);document.getElementById("sentenceTarget").insertAdjacentHTML("beforeend",`<span class="placed-word">${escapeHtml(button.dataset.word)}</span>`);if(sentenceChoice.length===sentenceWords.length){if(sentenceChoice.join(" ")===sentenceWords.join(" ")){setFeedback(document.getElementById("sentenceFeedback"),"Richtig! Im Herbst fallen die BlÃ¤tter.","success");unlockNext(8);}else setFeedback(document.getElementById("sentenceFeedback"),"Die Reihenfolge stimmt noch nicht. Versuche es noch einmal.","error");}}
...
function renderQuiz(){
  ...
  const submit=document.getElementById("submitQuiz");submit.disabled=false;submit.dataset.mode="check";submit.textContent="Quiz prÃ¼fen";
}
function submitQuiz(){
  ...
  if(submitButton.dataset.mode==="retry"){
    ...
    submitButton.textContent="Quiz prÃ¼fen";
