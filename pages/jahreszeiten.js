...
function playAudio(file,button){
  ...
  audio.pause();audio.currentTime=0;audio.src=AUDIO+file;
  if(button)button.classList.add("playing");
  audio.play().catch(()=>{if(button)button.classList.remove("playing");});
  audio.onended=()=>button&&button.classList.remove("playing");
  audio.onerror=()=>button&&button.classList.remove("playing");
}
...
function renderMonthGroups(){
  const box=document.getElementById("monthGroups");
  box.innerHTML=Object.entries(seasons).map(([key,s])=>`<button class="month-group" data-key="${key}" type="button"><h3>${s.name}</h3><p>${s.months.join(" · ")}</p></button>`).join("");
}
...
function setupObject(){
  ...
  document.getElementById("objectCard").innerHTML=`<img src="${IMAGE}${file}" alt="${escapeHtml(name)}"><strong>${escapeHtml(name)}</strong>`;
...
function checkObject(button){
  ...
  button.classList.add("correct");setFeedback(document.getElementById("objectFeedback"),`Richtig! ${objects[objectIndex][1]} passt zu ${seasons[correct].name}.`,"success");
  setTimeout(()=>{objectIndex++;if(objectIndex<8){setupObject();setFeedback(document.getElementById("objectFeedback"),"");}else{setFeedback(document.getElementById("objectFeedback"),"Gut gemacht! Der Koffer ist gepackt.","success");unlockNext(6);}},600);
}
...
function setupDress(){
  ...
  document.getElementById("dressFelix").src=IMAGE+`felix-${target}.webp`;
  document.getElementById("outfitOptions").innerHTML=shuffle(order).map(k=>`<button class="outfit-btn" data-value="${k}" type="button"><img src="${IMAGE}felix-${k}.webp" alt="Felix: ${seasons[k].name}"><span>${seasons[k].name}</span></button>`).join("");
}
...
document.addEventListener("click",event=>{
  const start=event.target.closest("#startLesson");if(start){document.querySelector(".hero").classList.add("completed");showStage(1);playAudio("intro.mp3");return;}
  ...
  const monthGroup=event.target.closest(".month-group");if(monthGroup){document.querySelectorAll(".month-group").forEach(b=>b.classList.toggle("active",b===monthGroup));playAudio(seasons[monthGroup.dataset.key].sentenceAudio);return;}
...
renderSeason("fruehling");renderMonthGroups();renderOrdinals();setupRecognition();setupSorting();setupObject();setupDress();setupSentence();renderQuiz();updateProgress(0);
